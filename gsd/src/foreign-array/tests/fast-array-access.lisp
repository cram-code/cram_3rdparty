...
;; Sebastian Sturm
;; Time-stamp: <2010-11-07 17:41:11EST fast-array-access.lisp>
	...
	(do ((m 0 (1+ m))) ((> m i)) (declare (fixnum m))
	  (do ((n i (1+ n))) ((= n dim)) (declare (fixnum n))
	    ...
	    (do ((k m (1+ k))) ((> k n)) (declare (fixnum k))
	      (incf (aref temp-values 1) (grid:gref (the vector-double-float zvector) k)))
	    (incf (aref temp-values 0) (expt (aref temp-values 1) -2))))
	(setf (grid:gref output i) 
	      (- (grid:gref (the vector-double-float zvector) i)
		 (aref temp-values 0)))))))
  ...
  (let ((temp-values (make-array 2 :element-type 'double-float :initial-element 0.0d0)))
	...
	(do ((m 0 (1+ m))) ((> m i)) (declare (fixnum m))
	  (do ((n i (1+ n))) ((= n dim)) (declare (fixnum n))
	    (setf (aref temp-values 1) 0.0d0)
	    (do ((k m (1+ k))) ((> k n)) (declare (fixnum k))
	      (incf (aref temp-values 1)
		    (grid:gref* (the vector-double-float zvector) k)))
	    (incf (aref temp-values 0) (expt (aref temp-values 1) -2))))
	(setf (grid:gref* output i) 
	      (- (grid:gref* (the vector-double-float zvector) i)
		 (aref temp-values 0)))))))
...
(let* ((dim 30)
       ...
       (slow-fn (gref-access dim))
       (intermediate-fn (gref*-access dim))
...
(time (let ((dim 50))
	(call-gref (grid:make-foreign-array 'double-float :initial-element 1 :dimensions dim)
